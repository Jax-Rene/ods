$(function () {
	...
	/*gloval variable*/
	image = json.img;
	image_num = image.length;
	col_num = 4;
	row_num = 4;
	count = image_num / col_num;
	current = 0;
	page_num = ((image_num % (row_num * col_num)) == 0) ? (image_num / (row_num * col_num)) : (parseInt(image_num / (row_num * col_num)) + 1);
	cur_page  = 0;
	/*init*/
	...
	/*img-show*/
	for(var i = 0; (i < col_num * row_num) && (i < image_num) ; i ++) {
		var content = 
			'<div class=\"img-cell\">'
				+'<img src=\"'+image[i].img_url+'\">'	
				+'<div class=\"img-details\">'
					+'<div class="text-details">'
						+'<p>姓名：'+image[i].name+'</p>'
						+'<p>公司：'+image[i].company+'</p>'
						+'<div>'
							+'<span class=\"congratulation\">'+image[i].congratulation+'</span>'
						+'</div>'
					+'</div>'
				+'</div>'		
			+'</div>';
	...
	$('.next').click(function () {
		...
		if(current < image_num) {
			$('.img-cell').remove();
			for( var i = current; (i < (current + col_num * row_num) && (i < image_num)); i ++){
				var content = 
					'<div class=\"img-cell\">'
						+'<div class=\"cover\">'+'</div>'	
						+'<img src=\"'+image[i].img_url+'\">'
						+'<div class=\"img-details\">'
							+'<div class="text-details">'
								+'<p>姓名：'+image[i].name+'</p>'
								+'<p>公司：'+image[i].company+'</p>'
								+'<div><img  class=\"dot\" src=\"./img/dot.jpg\">'+'<span class=\"congratulation\">'+image[i].congratulation+'</span>'+'<img src=\"./img/dot.jpg\"></div>'
							+'</div>'
						+'</div>'						
					+'</div>';
				$('.img-table').append(content);
		...
		if(current == image_num) $('.next').css("display", "none");
	});	
