...
public class OrderController {
    ...
    @ResponseBody
    public Map<String, Integer> currentOrderCount(int groupId) {
        int orderId = orderDao.getLastOrderId(groupId);
    ...
    @ResponseBody
    public boolean deletePersonOrder(int id, int orderId, int userId, HttpSession session) {
        User curUser = (User) session.getAttribute("curUser");
        Order order = orderDao.getOrder(orderId);
        int groupId = order.getOrderGroup();
        int bossId = groupDao.getBossId(groupId);
        if (curUser.getId() != userId && bossId != curUser.getId()) {
            return false; //没有权限修改
        } else {
    ...
    public boolean editPersonOrder(PersonOrder personOrder, HttpSession session) {
        ...
        if (curUser.getId() != personOrder.getUserId() && bossId != curUser.getId()) {
            return false; //没有权限修改
        } else {
            //删除订单名存在的空格
    ...
    public boolean insertPersonOrder(PersonOrder personOrder, int groupId, HttpSession session) {
        if (personOrder.getOrderId() == 0) { //最新的订单
